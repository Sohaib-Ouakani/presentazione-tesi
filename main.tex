  ...
  \begin{columns}
    ...
    \column{0.55\textwidth}
      ...
      \begin{tikzpicture}[
        ...
        svc/.style={box, fill=thesisAccent!10, draw=thesisAccent},
        browser/.style={box, fill=thesisBlue!15, minimum width=6.6cm},
        native/.style={box, fill=thesisGray!25, minimum width=6.6cm, font=\scriptsize\bfseries},
        arr/.style={->, thick, color=thesisBlue},
        node distance=0.45cm and 0.3cm,
        arr_glue/.style={->, thick, dashed, color=thesisAccent},
      ]
        \node[browser] (browser) {Browser};
        \node[svc, below=0.5cm of browser, xshift=-2.35cm] (svc1) {Servizio C++};
        \node[svc, below=0.5cm of browser] (svc2) {Servizio C\#};
        \node[svc, below=0.5cm of browser, xshift=2.35cm] (svc3) {Servizio Go};
        \node[box, below=0.7cm of svc1] (t1) {Simulatore\\C++};
        \node[box, below=0.7cm of svc2] (t2) {Emulatore\\C\#};
        \node[box, below=0.7cm of svc3] (t3) {Binding\\Go};
        \node[draw=thesisGray, dashed, rounded corners=2pt, fit=(t1)(t2)(t3), inner sep=0.15cm, label={[font=\tiny, text=thesisGray]above:glue code — duplicato per linguaggio}] (gluebox) {};
        \node[native, below=0.55cm of t2] (native) {Librerie native};
        \draw[arr] (browser.south) -- ++(0,-0.2) -| (svc1.north);
        \draw[arr] (browser.south) -- (svc2.north);
        \draw[arr] (browser.south) -- ++(0,-0.2) -| (svc3.north);
        \draw[arr] (svc1) -- (t1);
        \draw[arr] (svc2) -- (t2);
        \draw[arr] (svc3) -- (t3);
        \draw[arr_glue] (svc1) -- (t1);
        \draw[arr_glue] (svc2) -- (t2);
        \draw[arr_glue] (svc3) -- (t3);
        \draw[arr] (t1.south) -- ++(0,-0.3) -| (native.north);
        \draw[arr] (t2.south) -- (native.north);
        \draw[arr] (t3.south) -- ++(0,-0.3) -| (native.north);
      \end{tikzpicture}
      \\[0.3em]
    {\scriptsize Tre linguaggi, tre stack, tante librerie native da collegare a mano}
      \end{center}
  ...
  \begin{columns}
    ...
    \column{0.55\textwidth}
      ...
      \begin{tikzpicture}[
        ...
        \node[browser] (browser) {Browser};
        \node[kotlin, below left=0.6cm and -1.5cm of browser] (ui) {UI Kotlin/JS\\(Compose Multiplatform)};
        \node[kotlin, below right=0.6cm and -1.2cm of browser] (svc) {Servizio Backend\\(Kotlin/Native)};
        \node[native, below=0.6cm of svc] (native) {Librerie native (CInterop)};
        \draw[arr] (browser.south) -- ++(0,-0.2) -| (ui.north);
        \draw[arr] (browser.south) -- ++(0,-0.2) -| (svc.north);
        % \draw[arr] (ui.east) -- (svc.west) node[midway, above, font=\tiny] {chiamate};
        \draw[arr] (svc.south) -| (native.north);
      \end{tikzpicture}
      \\[0.3em]
